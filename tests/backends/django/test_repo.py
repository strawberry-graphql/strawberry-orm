...
class TestRefListRepo:
    def test_create_denied_by_repo(self, setup_tables):
        data = _seed()
        orm = StrawberryORM("django", repos={DjTag: DenyAllTagRepo})
    ...
    def test_update_denied_by_repo(self, setup_tables):
        data = _seed()
        orm = StrawberryORM("django", repos={DjTag: DenyUpdateTagRepo})
        info = SimpleNamespace(context={})
    ...
    def test_delete_denied_by_repo(self, setup_tables):
        data = _seed()
        orm = StrawberryORM("django", repos={DjTag: DenyDeleteTagRepo})
        info = SimpleNamespace(context={})
    ...
    def test_unlink_denied_by_repo(self, setup_tables):
        data = _seed()
        orm = StrawberryORM("django", repos={DjTag: DenyUnlinkTagRepo})
        info = SimpleNamespace(context={})
    ...
    def test_scope_query_hides_objects(self, setup_tables):
        data = _seed()
        orm = StrawberryORM("django", repos={DjTag: ScopingTagRepo})
...
# models on multiple StrawberryORM("django") instances breaks `auto` / Node resolution)
# ---------------------------------------------------------------------------


_repo_node_orm = StrawberryORM("django")
