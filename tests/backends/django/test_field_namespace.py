"""Field namespace tests for the Django backend."""
...
class TestFieldNamespace(AbstractTestFieldNamespace):
    ...
    def _orm(self):
        return StrawberryORM.for_django(lazy_resolution="off", warn_missing_scope=False)
    ...
    def _execute(self, orm, PostType, query, context=None):
        @strawberry.type
        class Query:
            posts: list[PostType] = orm.field.auto()

        result = orm.schema(query=Query).execute_sync(query, context_value=context)
...
class TestScopeIsARowControl(AbstractTestScopeIsARowControl):
    ...
    def _schema(self, scope):
        orm = StrawberryORM.for_django(lazy_resolution="off", warn_missing_scope=False)
    ...
    def read_scoped_edge(self):
        result = self._schema(self._published_only).execute_sync(
            "{ users { posts { title } } }"
        )
        assert result.errors is None, result.errors
        return sorted(
            post["title"] for u in result.data["users"] for post in u["posts"]
        )
    ...
    def _probe(self, title):
        ...
        assert result.errors is None, result.errors
        return [u["name"] for u in result.data["users"]]
    ...
    def run_dataclass_scope(self):
        ...
        result = self._schema(PublishedScope()).execute_sync(
            "{ users { posts { title } } }"
        )
        assert result.errors is None, result.errors
        return sorted(
