...
class TestRelayConnectionFilteringAndOrdering:
    ...
    def test_filter_and_order_are_applied_before_pagination(self, seed, User):
        ...
        first_page = schema.execute_sync(query)
    ...
    def test_ordering_controls_connection_slice(self, seed, User):
        schema = self._build_schema(User)
    ...
    def test_custom_list_field_applies_filter_and_order(self, seed, User):
        schema = self._build_schema(User)
        result = schema.execute_sync(
            """
                {
