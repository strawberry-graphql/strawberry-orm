"""Auto-resolution tests for the Tortoise backend."""
...
class TestQueryScalarAutoResolution:
...
class TestQueryRelationshipAutoResolution:
    @pytest.mark.asyncio
    async def test_user_posts_resolved(self, execute, seed):
        data = await execute("{ users { name posts { title } } }")
        assert data == {
            "users": [
                {
                    "name": "Alice",
    ...
    @pytest.mark.asyncio
    async def test_post_tags_resolved(self, execute, seed):
    ...
    @pytest.mark.asyncio
    async def test_post_comments_resolved(self, execute, seed):
        data = await execute("{ posts { title comments { body } } }")
        assert data == {
            "posts": [
                {
                    "title": "Hello World",
                ...
                {"title": "GraphQL Guide", "comments": [{"body": "Great guide"}]},
    ...
    @pytest.mark.asyncio
    async def test_deep_nesting(self, execute, seed):
        data = await execute(
    ...
    @pytest.mark.asyncio
    async def test_empty_relationship(self, execute, seed):
        data = await execute("{ posts { title tags { name } } }")
        assert data == {
            "posts": [
                {"title": "Hello World", "tags": [{"name": "python"}]},
    ...
    @pytest.mark.asyncio
    async def test_fk_scalar_resolves(self, execute, seed):
        data = await execute("{ posts { title comments { postId authorId } } }")
        assert data == {
            "posts": [
                {
                    "title": "Hello World",
