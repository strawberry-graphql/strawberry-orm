...
class TestRelayConnectionFilteringAndOrdering:
    ...
    async def test_filter_and_order_are_applied_before_pagination(self, seed, User):
    ...
    async def test_ordering_controls_connection_slice(self, seed, User):
        schema = self._build_schema(User)
    ...
    async def test_custom_list_field_applies_filter_and_order(self, seed, User):
        schema = self._build_schema(User)
        result = await schema.execute(
            """
                {
