"""Shared tests for createNode/updateNode graph mutations."""
...
class AbstractTestNodeGraphMutations:
    def test_create_node_builds_nested_graph(self, node_execute, seed):
        data = node_execute(
            """
            mutation {
                createNode(input: {
    ...
    def test_update_node_supports_explicit_nested_update(self, node_execute, seed):
        data = node_execute(
            """
            mutation {
                updateNode(input: {
    ...
    def test_create_node_input_helper_can_power_custom_mutation(
    ...
    def test_update_node_input_helper_can_power_custom_mutation(
        self, node_execute, seed
    ):
        data = node_execute(
            """
    ...
    def test_update_node_reverse_many_can_link_existing_child(self, node_execute, seed):
        ...
        query_data = node_execute("{ posts { title comments { body } } }")
        posts = {
    ...
    def test_update_node_reverse_many_can_update_existing_child(
        self, node_execute, seed
    ):
        data = node_execute(
            """
    ...
    def test_update_node_reverse_many_ignores_missing_child_refs(
        self, node_execute, seed
    ):
        data = node_execute(
            """
    ...
    def test_update_node_missing_root_instance_returns_error(
        self, node_execute_result, seed
    ):
        result = node_execute_result(
            """
    ...
    def test_projected_create_node_allows_selected_branch(
        self, projected_node_execute, seed
    ):
        data = projected_node_execute(
            """
            mutation {
    ...
    def test_projected_update_node_allows_selected_branch(
        self, projected_node_execute, seed
    ):
        data = projected_node_execute(
            """
    ...
    def test_projected_update_node_uses_fixed_tag_policy(
        self, projected_node_execute, seed
    ):
        data = projected_node_execute(
            """
        ...
        query_data = projected_node_execute("{ tags { name } }")
        tag_names = {entry["name"] for entry in query_data["tags"]}
        assert "projected-replacement-tag" in tag_names
        assert "rust" not in tag_names

    def test_projected_create_node_blocks_deeper_omitted_branch(
        self, projected_node_execute_result, seed
    ):
        result = projected_node_execute_result(
            """
            mutation {
                projectedCreateNode(input: {
                    post: {
    ...
    def test_invalid_projection_key_raises(self, orm, Post, seed):
        @orm.type(Post)
