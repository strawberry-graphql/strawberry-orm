...
def _seed_django() -> None:
    ...
    from tests.backends.django.models import (
        Comment as DjComment, Post as DjPost, Tag as DjTag, User as DjUser,
    )
    ...
    DjPost.objects.create(id=3, title="Draft Post", body="Not published yet", is_published=False, author=bob)
...
def _seed_sqlalchemy() -> Any:
    ...
    py, gql, rs = Tag(id=1, name="python"), Tag(id=2, name="graphql"), Tag(id=3, name="rust")
    session.add_all([py, gql, rs])
    session.flush()

    p1 = Post(id=1, title="Hello World", body="First post", is_published=True, author_id=1)
    p2 = Post(id=2, title="GraphQL Guide", body="Learn GraphQL", is_published=True, author_id=1)
    p3 = Post(id=3, title="Draft Post", body="Not published yet", is_published=False, author_id=2)
    p4 = Post(id=4, title="Rust Adventures", body="Systems programming", is_published=True, author_id=3)
...
def _load_schema(backend: Backend, name: str) -> Any:
    ...
    if schema is None:
        available = sorted(a.removesuffix("_schema") for a in dir(mod) if a.endswith("_schema"))
        typer.echo(f"Unknown schema '{name}'. Available: {', '.join(available)}", err=True)
        raise typer.Exit(1)
    return schema
...
def show(
    ...
    backend: Backend = typer.Option(Backend.django, "--backend", "-b", help="ORM backend."),
